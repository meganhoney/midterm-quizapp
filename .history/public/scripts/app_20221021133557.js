  ...
  */
  ...
  const createQuizRow = function (quiz) {
    ...
    let $quiz = `
      <tr>
        ...
        <td>${quiz.topic}</td>
        <td>${dateString}</td>
        <td><a href="http://localhost:8080/${quiz.id}" class="btn share-button" role="button">Share</a></td>
      </tr>
    `;
  ...
  const displayAllQuizzes = function (quizzes) {
    ...
    $shareButton.click((event) => {;
      ...
      let index = $( $shareButton).index( this );
      var listItems = $( ".share-button" ).slice( 1 );
alert( "Index: " + $( ".share-button" ).index( listItems ) );
      console.log($(this))
      let hrefValue = $(this)[0].attr('href');//get the href property of the current clicked element
